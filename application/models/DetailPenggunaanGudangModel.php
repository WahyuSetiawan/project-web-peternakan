...
class DetailPenggunaanGudangModel extends CI_Model
{
    ...
    public function select($id_pembelian_ayam = null, $params = [])
    {
        if (isset($params['gudang'])) {
            $this->db->where(GudangModel::$table . ".id_gudang", $params['gudang']);
        }

        if (isset($params['kandang'])) {
            $this->db->where(KandangModel::$table . ".id_kandang", $params['kandang']);
        }
    ...
    public function stokGudang($id_gudang)
    {
        $this->db->select(
            "jumlah_gudang"
        );
        $this->db->where("id_gudang", $id_gudang);
        $data = $this->db->get("view_stok_gudang")->row();
        return $data->jumlah_gudang;
    }
    ...
    public function belum($limit = false, $offset = false, $id_pembelian_ayam = false, $params = [])
    {
        ...
        $this->db->where("
            id_jadwal_kandang not in (select id_jadwal from tb_detail_penggunaan_gudang where date(tanggal) = date('" . $params['tanggal'] . "'))");
