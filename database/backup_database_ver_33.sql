...
--
-- Host: localhost
-- Generation Time: Aug 18, 2019 at 01:29 AM
...
INSERT INTO `tb_detail_penjualan_ayam` (`id_detail_penjualan_ayam`, `id_kandang`, `id_karyawan`, `id_admin`, `id_detail_group_transaksi`, `id_pemesanan`, `tanggal`, `jumlah_ayam`, `harga`, `keterangan`, `update_by_karyawan`, `update_by_admin`, `created_at`, `updated_at`) VALUES
('KA_0001', 'KD_0001', 'KR_0001', NULL, 'GT_0007', NULL, '2019-08-04 00:00:00', 10, 10, '', NULL, NULL, '2019-08-04 09:27:26', '0000-00-00 00:00:00'),
('KA_0002', 'KD_0001', 'KR_0001', NULL, 'GT_0007', NULL, '2019-08-04 00:00:00', 10, 10, '', NULL, NULL, '2019-08-04 10:19:27', '0000-00-00 00:00:00'),
('KA_0003', 'KD_0001', 'KR_0001', NULL, 'GT_0007', 1, '2019-08-18 00:00:00', 31, 10, '', NULL, NULL, '2019-08-17 23:22:03', '0000-00-00 00:00:00');
...
CREATE TABLE IF NOT EXISTS `tb_pemesanan` (
  ...
  `jumlah_ayam` int(11) NOT NULL,
  `status` enum('pesan','ditolak','seleai','pengiriman','') NOT NULL DEFAULT 'pesan',
...
INSERT INTO `tb_pemesanan` (`id_pemesanan`, `nama`, `telepon`, `alamat`, `jumlah_ayam`, `status`, `alasan`) VALUES
(1, 'wahyu', '08', 'pugeran', 50, 'pesan', NULL),
(2, 'wahyu', '08', 'pugeran', 11, 'ditolak', 'stok habis');
...
--
CREATE TABLE IF NOT EXISTS `view_detail_pemesanan` (
`id_pemesanan` int(11)
,`nama` varchar(20)
,`telepon` varchar(15)
,`alamat` text
,`jumlah_ayam` int(11)
,`status` enum('pesan','ditolak','seleai','pengiriman','')
,`alasan` varchar(50)
,`jumlah_transaksi` bigint(21)
,`jumlah_penjualan` decimal(32,0)
,`sisa_jumlah` decimal(33,0)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`user1`@`localhost` SQL SECURITY DEFINER VIEW `view_detail_pemesanan` AS select `tb_pemesanan`.`id_pemesanan` AS `id_pemesanan`,`tb_pemesanan`.`nama` AS `nama`,`tb_pemesanan`.`telepon` AS `telepon`,`tb_pemesanan`.`alamat` AS `alamat`,`tb_pemesanan`.`jumlah_ayam` AS `jumlah_ayam`,`tb_pemesanan`.`status` AS `status`,`tb_pemesanan`.`alasan` AS `alasan`,(select ifnull(count(0),0) from `tb_detail_penjualan_ayam` where (`tb_detail_penjualan_ayam`.`id_pemesanan` = `tb_pemesanan`.`id_pemesanan`)) AS `jumlah_transaksi`,(select ifnull(sum(`tb_detail_penjualan_ayam`.`harga`),0) from `tb_detail_penjualan_ayam` where (`tb_detail_penjualan_ayam`.`id_pemesanan` = `tb_pemesanan`.`id_pemesanan`)) AS `jumlah_penjualan`,(`tb_pemesanan`.`jumlah_ayam` - (select ifnull(sum(`tb_detail_penjualan_ayam`.`jumlah_ayam`),0) from `tb_detail_penjualan_ayam` where (`tb_detail_penjualan_ayam`.`id_pemesanan` = `tb_pemesanan`.`id_pemesanan`))) AS `sisa_jumlah` from `tb_pemesanan`;
...
ALTER TABLE `tb_detail_penjualan_ayam`
  ...
  ADD CONSTRAINT `tb_detail_penjualan_ayam_ibfk_9` FOREIGN KEY (`id_pemesanan`) REFERENCES `tb_pemesanan` (`id_pemesanan`) ON DELETE SET NULL ON UPDATE CASCADE;
