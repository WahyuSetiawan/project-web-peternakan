...
-- Dumping data for table peternakan_app.detail_kerugian_ayam: ~-1 rows (approximately)
DELETE FROM `detail_kerugian_ayam`;
/*!40000 ALTER TABLE `detail_kerugian_ayam` DISABLE KEYS */;
/*!40000 ALTER TABLE `detail_kerugian_ayam` ENABLE KEYS */;
...
CREATE TABLE IF NOT EXISTS `detail_type_supplier` (
  ...
  KEY `FK_detail_type_supplier_type_supplier` (`id_persediaan`),
  ...
  CONSTRAINT `FK_detail_type_supplier_type_supplier` FOREIGN KEY (`id_persediaan`) REFERENCES `type_supplier` (`id_type_supplier`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `detail_type_supplier` (`id_detail_type_supplier`, `id_supplier`, `id_persediaan`) VALUES
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `function_id_persediaan`() RETURNS varchar(10) CHARSET latin1
begin
	declare id INT;
	declare count_id INT;
	declare code varchar(11);
...
INSERT INTO `persediaan` (`id_persediaan`, `nama`, `cara_pemakaian`, `created_at`, `updated_at`) VALUES
...
CREATE TABLE `view_stok_persediaan` (
	...
	`nama_persediaan` VARCHAR(50) NULL COLLATE 'latin1_swedish_ci',
...
CREATE TABLE `view_transaksi_kandang` (
	...
	`tanggal` DATETIME NOT NULL,
	`jumlah_ayam` INT(11) NOT NULL,
	`keterangan` VARCHAR(9) NOT NULL COLLATE 'utf8_general_ci',
	`aksi` VARCHAR(2) NOT NULL COLLATE 'utf8_general_ci',
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_stok_ayam` AS select `kandang`.`id_kandang` AS `id_kandang`,`kandang`.`nama` AS `nama`,((ifnull((select sum(`detail_pembelian_ayam`.`jumlah_ayam`) from `detail_pembelian_ayam` where (`detail_pembelian_ayam`.`id_kandang` = `kandang`.`id_kandang`)),0) - ifnull((select sum(`detail_penjualan_ayam`.`jumlah_ayam`) from `detail_penjualan_ayam` where (`detail_penjualan_ayam`.`id_kandang` = `kandang`.`id_kandang`)),0)) - ifnull((select sum(`detail_kerugian_ayam`.`jumlah`) from `detail_kerugian_ayam` where (`detail_kerugian_ayam`.`id_kandang` = `kandang`.`id_kandang`)),0)) AS `jumlah`,(select count(`view_transaksi_kandang`.`id_transaksi`) from `view_transaksi_kandang` where (`view_transaksi_kandang`.`id_kandang` = `kandang`.`id_kandang`)) AS `jumlah_transaksi`,(select count(`view_transaksi_kandang`.`id_transaksi`) from `view_transaksi_kandang` where ((`view_transaksi_kandang`.`id_kandang` = `kandang`.`id_kandang`) and (`view_transaksi_kandang`.`aksi` = 'in'))) AS `jumlah_transaksi_masuk`,(select count(`view_transaksi_kandang`.`id_transaksi`) from `view_transaksi_kandang` where ((`view_transaksi_kandang`.`id_kandang` = `kandang`.`id_kandang`) and (`view_transaksi_kandang`.`aksi` = 'out'))) AS `jumlah_transaksi_keluar`,(select count(`view_transaksi_kandang`.`id_transaksi`) from `view_transaksi_kandang` where ((`view_transaksi_kandang`.`id_kandang` = `kandang`.`id_kandang`) and (`view_transaksi_kandang`.`aksi` = 'die'))) AS `jumlah_kerugian` from `kandang`;

-- Dumping structure for view peternakan_app.view_stok_persediaan
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `view_stok_persediaan`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_stok_persediaan` AS select `persediaan`.`id_persediaan` AS `id_persediaan`,`persediaan`.`nama` AS `nama_persediaan`,(ifnull((select sum(`view_transaksi_persediaan`.`jumlah`) from `view_transaksi_persediaan` where ((`view_transaksi_persediaan`.`id_persediaan` = `persediaan`.`id_persediaan`) and (`view_transaksi_persediaan`.`aksi` = 'in'))),0) - ifnull((select sum(`view_transaksi_persediaan`.`jumlah`) from `view_transaksi_persediaan` where ((`view_transaksi_persediaan`.`id_persediaan` = `persediaan`.`id_persediaan`) and (`view_transaksi_persediaan`.`aksi` = 'out'))),0)) AS `jumlah_persediaan`,(select count(`view_transaksi_persediaan`.`jumlah`) from `view_transaksi_persediaan` where (`view_transaksi_persediaan`.`id_persediaan` = `persediaan`.`id_persediaan`)) AS `jumlah_transaksi`,(select count(`view_transaksi_persediaan`.`jumlah`) from `view_transaksi_persediaan` where ((`view_transaksi_persediaan`.`id_persediaan` = `persediaan`.`id_persediaan`) and (`view_transaksi_persediaan`.`aksi` = 'in'))) AS `jumlah_transaksi_masuk`,(select count(`view_transaksi_persediaan`.`jumlah`) from `view_transaksi_persediaan` where ((`view_transaksi_persediaan`.`id_persediaan` = `persediaan`.`id_persediaan`) and (`view_transaksi_persediaan`.`aksi` = 'out'))) AS `jumlah_transaksi_keluar` from `persediaan`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_transaksi_persediaan` AS (select `detail_pembelian_gudang`.`id_detail_pembelian_gudang` AS `id_transaksi`,'in' AS `aksi`,`detail_pembelian_gudang`.`id_persediaan` AS `id_persediaan`,`detail_pembelian_gudang`.`id_supplier` AS `id_supplier`,`detail_pembelian_gudang`.`tanggal` AS `tanggal`,`detail_pembelian_gudang`.`jumlah` AS `jumlah`,`detail_pembelian_gudang`.`id_admin` AS `id_admin`,`detail_pembelian_gudang`.`id_karyawan` AS `id_karyawan`,`detail_pembelian_gudang`.`update_by_admin` AS `update_by_admin`,`detail_pembelian_gudang`.`update_by_karyawan` AS `update_by_karyawan`,`detail_pembelian_gudang`.`created_at` AS `created_at`,`detail_pembelian_gudang`.`updated_at` AS `updated_at`,'' AS `keterangan` from `detail_pembelian_gudang`) union all (select `detail_pengeluaran_gudang`.`id_detail_pengeluaran_gudang` AS `id_detail_pengeluaran_gudang`,'out' AS `aksi`,`detail_pengeluaran_gudang`.`id_persediaan` AS `id_persediaan`,NULL AS `id_supplier`,`detail_pengeluaran_gudang`.`tanggal` AS `tanggal`,`detail_pengeluaran_gudang`.`jumlah` AS `jumlah`,`detail_pengeluaran_gudang`.`id_admin` AS `id_admin`,`detail_pengeluaran_gudang`.`id_karyawan` AS `id_karyawan`,`detail_pengeluaran_gudang`.`update_by_admin` AS `update_by_admin`,`detail_pengeluaran_gudang`.`update_by_karyawan` AS `update_by_karyawan`,`detail_pengeluaran_gudang`.`created_at` AS `created_at`,`detail_pengeluaran_gudang`.`updated_at` AS `updated_at`,`detail_pengeluaran_gudang`.`keterangan` AS `keterangan` from `detail_pengeluaran_gudang`);
